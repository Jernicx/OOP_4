...
namespace OOP_4
{
        public partial class Form1 : Form
        {
            ...
            public class MyStorage
            {
                ...
                public void remove()                            // удаляет все обнуленные элементы
            {
                    int del = 0;
                    ...
                    CCircle[] tempStorage = new CCircle[del];   //сюда мы поместим элементы, которые должны остаться

                int j = 0;
                    for (int i = 0; i < size; i++)
                        if (storage[i] != null)
                        {
                            tempStorage[j] = storage[i];        // размещение оставшихся элементов
                        j = j + 1;
                        }

                    size = del;                                 // изменение свойств
                  count = size;
                    iter = size;
                    if (iter < 0)
                        iter = 0;
                    ...
                    for (int i = 0; i < size; i++)
                        storage[i] = tempStorage[i];            // переместил все оставшиеся элементы
            }
                ...
                public void removeFocused(Graphics paintForm)
                {
                    for (int i = 0; i < size; i++)
                    {
                        if (storage[i].focusCheck() == true)
                            storage[i] = null;                  // помещая null в хранилище в элементах, которые мы должны удалить
                }

                    remove();

                // теперь на рисования формы мы не будем рисовать элементы, которые были сфокусированы.
                ActiveForm.Invalidate();
                }
            ...
            {
            //PointToClient возвращает положение мыши по отношению к форме, а не к экрану
